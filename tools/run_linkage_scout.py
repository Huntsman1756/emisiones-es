...
man = json.load(open('g0/manifests/linkage-scout.json', encoding='utf-8'))
...
def mk_src(s):
    ...
    return SourceRecord(
        ...
        version_suffix=o.get('version_suffix'),
        rol=o.get('rol'), isin=s.get('isin') or o.get('isin'),
        doc_url=s.get('doc_url') or o.get('doc_url'),
        numfol_link=o.get('numfol_link'),
        emisor=o.get('emisor'))
...
out = {'manifest': 'linkage-scout.json',
       'manifest_sha256': man['manifest_sha256'],
       'note': 'SCOUT METRICS != G0 VERDICT; thresholds de holdout intactos',
       'metrics': metrics,
       'per_stratum': {f'{s}|{"ok" if ok else "fail"}': n
                       for (s, ok), n in sorted(by_stratum.items())},
       'cases': results}
json.dump(out, open('g0/results/linkage-scout-results.json', 'w',
